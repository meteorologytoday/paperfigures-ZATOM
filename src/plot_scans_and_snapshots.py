import matplotlib as mplt

#mplt.use("TkAgg")

import matplotlib.gridspec as gridspec
from matplotlib import cm
from matplotlib import rc
import os
import re
import pprint
default_linewidth = 1.0;
default_ticksize = 10.0;

mplt.rcParams['lines.linewidth'] =   default_linewidth;
mplt.rcParams['axes.linewidth'] =    default_linewidth;
mplt.rcParams['xtick.major.size'] =  default_ticksize;
mplt.rcParams['xtick.major.width'] = default_linewidth;
mplt.rcParams['ytick.major.size'] =  default_ticksize;
mplt.rcParams['ytick.major.width'] = default_linewidth;

rc('font', **{'size': 15.0});
rc('axes', **{'labelsize': 15.0});
rc('mathtext', **{'fontset':'stixsans'});

import matplotlib.pyplot as plt

import os,sys, argparse
from netCDF4 import Dataset
import numpy as np


def repNaN2None(arr):
    for i, elm in enumerate(arr):
        if np.isnan(elm):
            arr[i] = None



parser = argparse.ArgumentParser()
parser.add_argument('--folder', nargs='+')
parser.add_argument('--legend', nargs='*')
parser.add_argument('--output-bifur', default="")
parser.add_argument('--output-marks', default="")
parser.add_argument('--F-rng', nargs=2, type=float)
parser.add_argument('--psi-rng', nargs=2, type=float, default=[np.nan, np.nan])
parser.add_argument('--numbering-jmp', type=int, default=10)
parser.add_argument('--show-numbering', action="store_true")
parser.add_argument('--show-filename', action="store_true")
parser.add_argument('--metric', type=str, default="max", choices=["max", "60N", "40N", "20N"])
parser.add_argument('--marks', nargs='*', type=float, default=[])
args = parser.parse_args()

pp = pprint.PrettyPrinter(indent=4)
pp.pprint(args)

repNaN2None(args.psi_rng)
data = []
coor = {}


folders = args.folder
legends   = args.legend

if (legends is None) or (legends is not None and len(legends) < len(folders)):
    print("Legends do not have the same length of folders. Use numbering instead")
    legends = ["%d" % (i,) for i in range(len(folders))]

print(legends)

if len(args.marks) % 2 != 0:
    raise Exception("Length of mark is not multiple of 2. It has to be a list of (x, y) pairs. ")


# Data contains a list of folders (cases)
# and each case has multiple data files

for (i, folder) in enumerate(folders):
   
    print("Loading data in folder %s" % (folder,)) 
    
    filenames = os.listdir(folder)
    filenames.sort()
 
    valid_filenames = []
 
    data_in_folder = []
    for (j, filename) in enumerate(filenames):
        m = re.search(r'^(?P<num>[0-9]{4}).nc$', filename)
        if m is not None:
            num = int(m.group('num'))
            
            if num == 0:
                print("Skip %s" % filename)
                continue

            valid_filenames.append(filename)

            d = {}
            d["filename"]  = filename
            with Dataset("%s/%s" % (folder, filename), "r") as f:
               
                if "y_V" not in coor:
                    for varname in ["y_V", "z_W", "y_T", "z_T"]:
                        coor[varname] = f.variables[varname][:]

                #for varname in ["heat_transport", "Psib", "Q", "res", "cvg"]:
                for varname in ["Psib", "Q", "res", "be", "bw", "vw", "ww", "we", "ui"]:
                    d[varname] = f.variables[varname][:]
                    print("Shape of variable %s : " % (varname,), d[varname].shape)


            
            data_in_folder.append(d)

    data.append(data_in_folder)        

coor["y_V"] *= 180.0/np.pi
coor["y_T"] *= 180.0/np.pi
coor["z_T"] *= -1 / 1e3
coor["z_W"] *= -1 / 1e3

coor["z_T"] = coor["z_T"]
coor["z_W"] = coor["z_W"]

print("Shape of z : ", coor["z_W"].shape)

res_threshold = 1e-12#1.0 * (9.81*2e-4) / (86400*360*10)
print("res_threshold = %.3e" % (res_threshold,))

y_ind = 0
if args.metric == "20N":
    y_ind = np.argmin(np.abs(coor["y_T"] - 20.0))
elif args.metric == "40N":
    y_ind = np.argmin(np.abs(coor["y_T"] - 40.0))
elif args.metric == "60N":
    y_ind = np.argmin(np.abs(coor["y_T"] - 60.0))


# look for proper z coordinate
z_ind = np.argmin(np.abs(coor["z_W"] - 1000.0))

print("z_ind = %d, y_ind = %d" % (z_ind, y_ind)) 

# Processing data
for folder in data:
    for d in folder:
        d["Q"] /= 1e6
        d["Psib"] /= 1e6

        if args.metric == "max":
           d["psi_measure"] = np.amax(d["Psib"][:, :, :], axis=(1,2))
        elif args.metric in ["20N", "40N", "60N"]:
            #psi = np.amax(d["Psib"][:, :, y_ind], axis=1)
           d["psi_measure"] = d["Psib"][:, y_ind, z_ind]
 


# Compute the deep water formation grids

def computeDeepWaterFormationGrids(b):

    flags = b * 0
    b_lower = np.roll(b, -1, axis=1)
    
    flags[b_lower > b] = 1.0
    flags[:, -1] = flags[:, -2]
    
    return flags

for folder in data:
    for d in folder:

        Q = d["Q"]
        be = d["be"]
        bw = d["bw"]
       
        dwf_east = be * 0.0
        dwf_west = be * 0.0
        for j in range(len(Q)-1):

            dwf_east[j, :, :] = computeDeepWaterFormationGrids(be[j, :, :])
            dwf_west[j, :, :] = computeDeepWaterFormationGrids(bw[j, :, :])

        d["dwf_east"] = dwf_east
        d["dwf_west"] = dwf_west

        #print(np.sum(dwf_east))

# Pick out the marks
def dist(x1,y1,x2,y2):
    return ((x1-x2)**2 + (y1-y2)**2)**0.5

nmarkpairs = int( len(args.marks) / 2 )
shortest_dist = np.zeros((nmarkpairs,)) + 1e5
mark_index = np.zeros((nmarkpairs, 3), dtype=int) - 1
        
for k in range(nmarkpairs):
    print("Search for the marker case closest to : (%f, %f)" % (args.marks[k*2], args.marks[k*2+1]))
    for i, _folders in enumerate(data):
        for j, d in enumerate(_folders):
            for s in range(len(d["Q"])):
                _dist = dist(d["Q"][s], d["psi_measure"][s], args.marks[k*2], args.marks[k*2+1])
                if _dist < shortest_dist[k]:
                    shortest_dist[k] = _dist
                    mark_index[k, 0] = i
                    mark_index[k, 1] = j
                    mark_index[k, 2] = s
    print("Found: (%d, %d, %d)" % (mark_index[k, 0], mark_index[k, 1], mark_index[k, 2]))


                
print("Data loaded. Plotting now...")

colors = [
    'red',
    'orange',
    'green', 
    'blue',
    'violet',
    'black',
    "grey",
    "dodgerblue",
]

fig, ax = plt.subplots(1, 1, figsize=(6, 6), squeeze=False, constrained_layout=True)

for i in range(len(legends)):
    
    print("Plotting %s (%s)" % (legends[i], folders[i]))

    data_in_folder = data[i]
    offset = 0

    Q           = np.array([], dtype=np.float64)
    res         = np.array([], dtype=np.float64)
    psi_measure = np.array([], dtype=np.float64)
    
    for (j, d) in enumerate(data_in_folder):

        Q   = np.concatenate((Q, d["Q"]))
        res = np.concatenate((res, d["res"]))
        psi_measure = np.concatenate((psi_measure, d["psi_measure"]))


    kw_label = {}
    kw_label['label'] = legends[i]

    # remove large jump
    for j in range(len(Q)-1):

        dQ = Q[j+1] - Q[j]
        dpsi_measure = psi_measure[j+1] - psi_measure[j]
        if abs(dQ) > 0.05 or abs(dpsi_measure) > 1.0:
            # drop all points after this
            Q = Q[:j+1]
            res = res[:j+1]
            psi_measure = psi_measure[:j+1]

            break
    
    ax[0, 0].plot(Q, psi_measure, color="black", zorder=1)


    """
    if args.show_numbering == True:
        for text_idx in range(0,len(Q),args.numbering_jmp):
            ax[0].text(Q[text_idx], psi[text_idx], "%d" % (text_idx+1), size=8)
    """
    


for k in range(nmarkpairs):
    print("Marking the %d-th marker." % k)
    d = data[mark_index[k,0]][mark_index[k,1]]
    s = mark_index[k, 2]
    ax[0,0].scatter(d["Q"][s], d["psi_measure"][s], s=80, marker="*", c="yellow", edgecolor="k", zorder=50)
    ax[0,0].text(d["Q"][s] - 0.03, d["psi_measure"][s], "%d" % (k+1,), size=15, va="center", ha="right")


#ax[0, 0].legend()
#cb = plt.colorbar(mappable, ax=ax[0,0])
#cb.ax.set_ylabel("$ \\log\\left(R\\right) $ ")
#cb.ax.set_yticks([-11, -10, -9, -8])

#ax[0].set_title("Heat transport [PW]")
if args.metric == "30N":
    ax[0, 0].set_ylabel(r"$\Psi$ at $30^{\circ}\mathrm{N}$ 1000m depth [Sv]")

elif args.metric == "max":
    ax[0, 0].set_ylabel(r"$\Psi$ maximum [Sv]")

#ax[0].set_ylim([-0.10, 0.25])
#ax[1].set_ylim([0, 1e-11])

ax[0, 0].set_ylim(args.psi_rng)
    
ax[0, 0].set_xlabel("$\\gamma$ [Sv]")
ax[0, 0].set_xlim(args.F_rng)

#ax[1].set_xlabel("Count")
#ax[1].set_ylabel("Residue")
#ax[1].plot([0, len(Q)], [res_threshold]*2, "r--")



if args.output_bifur != "":
    fig.savefig(args.output_bifur, dpi=300)


if nmarkpairs != 0:
    fig2, ax2 = plt.subplots(3, nmarkpairs, sharey=False, sharex=True, figsize=(4*nmarkpairs, 4*3), gridspec_kw={'bottom':0.2}, squeeze=False)
   
    levels_b = np.arange(0, 100, 2)
    levels_ui = np.arange(-100, 100, 0.5)
    levels_we = np.arange(-100, 100, .5)
    levels_ww = np.arange(-100, 100, .5)
    #dwf_color = (1.0, 0.7, 0.7)
    dwf_color = (0.6, 0.75, 1.0)
 
    for k in range(nmarkpairs):
        
        print("Max and min of bw: %.2e ; %.2e" % (np.amax(d["bw"]), np.amin(d["bw"])))
        print("Max and min of ui: %.2e ; %.2e" % (np.amax(d["ui"]), np.amin(d["ui"])))
        print("Max and min of we: %.2e ; %.2e" % (np.amax(d["we"]), np.amin(d["we"])))
        print("Max and min of ww: %.2e ; %.2e" % (np.amax(d["ww"]), np.amin(d["ww"])))

        _ax = ax2[:, k]
        print("Plotting the %d-th marker streamfunction" % k)
        d = data[mark_index[k,0]][mark_index[k,1]]
        s = mark_index[k, 2]
        
        CS = _ax[0].contour(coor["y_V"], coor["z_W"], d["Psib"][s, :, :].transpose(), range(-20, 20, 2), colors="black")
        _ax[0].clabel(CS, CS.levels, inline=True, fmt="%d", fontsize=10, inline_spacing=4)
        
     
           
        #CS = _ax[1].contour(coor["y_T"], coor["z_T"] / 1e3, d["ui"][s, :, :].transpose() * 100, levels_ui, colors="red")
        CS = _ax[1].contour(coor["y_T"], coor["z_W"], d["ww"][s, :, :].transpose() * 1e5, levels_ww, colors="red")
        _ax[1].clabel(CS, CS.levels, inline=True, fmt="%.1f", fontsize=10, inline_spacing=2)
        CS = _ax[1].contour(coor["y_T"], coor["z_T"], ( d["bw"][s, :, :].transpose() - 0.20 ) * 1000, levels_b, colors="black")
        _ax[1].clabel(CS, CS.levels, inline=True, fmt="%.1f", fontsize=10, inline_spacing=2)
        #cs_dwf_west = _ax[1].contourf(coor['y_T'], coor['z_T'], d['dwf_west'][s, :, :].transpose(), [0, 0.5,1.5], colors=['none']*2, hatches=[None, ".."])
        cs_dwf_west = _ax[1].contourf(coor['y_T'], coor['z_T'], d['dwf_west'][s, :, :].transpose(), [0, 0.5,1.5], colors=['none', dwf_color])


        CS = _ax[2].contour(coor["y_T"], coor["z_W"], d["we"][s, :, :].transpose() * 1e5, levels_we, colors="red")
        _ax[2].clabel(CS, CS.levels, inline=True, fmt="%.1f", fontsize=10, inline_spacing=2)
        
        CS = _ax[2].contour(coor["y_T"], coor["z_T"], ( d["be"][s, :, :].transpose() - 0.20 ) * 1000, levels_b, colors="black")
        _ax[2].clabel(CS, CS.levels, inline=True, fmt="%d", fontsize=10, inline_spacing=2)
        #cs_dwf_east = _ax[2].contourf(coor['y_T'], coor['z_T'], d['dwf_east'][s, :, :].transpose(), [0, 0.5,1.5], colors=['none']*2, hatches=[None, "..."])
        cs_dwf_east = _ax[2].contourf(coor['y_T'], coor['z_T'], d['dwf_east'][s, :, :].transpose(), [0, 0.5,1.5], colors=['none', dwf_color])


        #for _cs in [cs_dwf_east, cs_dwf_west]:
        #    for _, collection in enumerate(_cs.collections):
        #        collection.set_edgecolor((1.0, 0.5, 0.5))
        #        collection.set_linewidth(0.0)
 


        _ax[0].set_title("State %s ($%.2f \\, \\mathrm{Sv}$)" % ("%d" % (k+1,), d["Q"][s]))

        if k == 0:
            for __ax in _ax:
                __ax.set_ylabel("Depth [km]")
        
        _ax[-1].set_xlabel("Latitude [${}^\\circ\\mathrm{N}$]")

        for __ax in _ax:
            __ax.set_xticks([20,30,40,50,60])
            __ax.set_yticks([0, 1, 2, 3, 4])

            if k != 0:
                __ax.set_yticklabels([""] * len(__ax.get_yticks()))

        _ax[0].set_ylim([4.5, 0])
        _ax[1].set_ylim([2.0, 0])
        _ax[2].set_ylim([2.0, 0])

    if args.output_marks != "":
        fig2.savefig(args.output_marks, dpi=300)



plt.show()









