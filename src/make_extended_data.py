...
def makeExtendedData2(data, coor, lat_s = 40.0, lat_n = 68.0, verbose = False):
    ...
    verbose and print("Integrate only the range between lat [%f, %f], with idx founc [%d, %d]" % (
        lat_s, lat_n,
        lat_s_ind, lat_n_ind,
    ))
    ...
    # Here I want to compute the exact terms
    ...
    dbwdy_V = ( data["bw"][:, 1:, :] - data["bw"][:, :-1, :] ) / ( coor["dy_V"][1:-1][None, None, :] )
    
    eff_dbdz_W = Lambda * dbedz_W + dbwdz_W
    new_data["mode1_dbdiffdt_dueto_ZOC"] = yavg(
        mode_integration(
            w_ZOC[:, :, 1:-1] * eff_dbdz_W,
            wgt=mode_1_dz_weight_W[1:-1],
        ),
        wgt=coor["cos_lat"],
    )
